...
// https://sdkwa.pro/docs/api/receiving/notifications-format/DeviceInfo/
//
// Параметры,
//  Отказ - Булево -  Если Истина, то сообщение не будет подтверждено и вернется на сервер 
//    для повторного получения
...
// Подробнее https://cabinet.sdkwa.pro/docs
// 
...
//
// Параметры:
//  СтруктураНастроек - Структура - См описание метода https://cabinet.sdkwa.pro/docs
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Метод предназначен для получения кода  для авторизации.
// Подробнее https://cabinet.sdkwa.pro/docs
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ПолучитьКодРегистрации(НомерТелефона, Метод) Экспорт
	...
	Ответ = ОтправитьPOSTЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "getAuthorizationCode"), Тело, Ложь);
	Возврат Ответ;
...
Функция ОтправитьТекстВГруппу(ИдЧата, Сообщение) Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("chatId", ПараметрИдЧата(ИдЧата));
	Структура.Вставить("message", Сообщение);
...
Функция ПолучитьИнформациюОГруппе(ИдГруппы) Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("groupId", ПараметрИдЧата(ИдГруппы));
...
Функция ДобавитьУчастникаВГруппу(ИдГруппы, ИдУчастника) Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("groupId", ИдГруппы);
	Структура.Вставить("participantChatId", ПараметрИдЧата(ИдУчастника));
...
Функция ОтправитьФайлPOSTЗапрос(Хост, Адрес, Заголовки, ТелоЗапроса, Защищенное = Истина)
	
	Если Защищенное Тогда
		Сертификат = Новый ЗащищенноеСоединениеOpenSSL(,, РежимПроверкиОтзываTLSСертификатаСервера.НеПроверять);
		Соединение = Новый HTTPСоединение(Хост,,,,,10, Сертификат);
	Иначе
		Соединение = Новый HTTPСоединение(Хост,,,,,10);
	КонецЕсли;
	
	ВебЗапрос = Новый HTTPЗапрос(Адрес,Заголовки);
	ВебЗапрос.Заголовки.Вставить("Authorization", "Bearer " + ApiToken);
...
Функция ОтправитьPOSTЗапрос(Хост, Адрес, Тело, ВызватьОшибку = Истина, Защищенное = Истина)
	...
	Иначе
		...
		Иначе
			Чтение = Новый ЧтениеJSON();
			Чтение.УстановитьСтроку(ТекстОшибки);
			ПрочитанныйОтвет = ПрочитатьJSON(Чтение);
			Чтение.Закрыть();
			Возврат ПрочитанныйОтвет;
		КонецЕсли;
...
Функция URLМетодаСервиса(Инстанс, Токен, Метод)
	Возврат ПодставитьПараметрыВСтроку("whatsapp/%1/%2", Инстанс, Метод);
КонецФункции
...
Функция ПараметрИдЧата(ТелефонЧатИд)
	
	Если ТипЗнч(ТелефонЧатИд) = Тип("Число") Тогда
		Возврат Формат(Число(ТелефонЧатИд), "ЧГ=") + "@c.us"
	Иначе
		Если СтрНайти(ТелефонЧатИд, "@c.us") Или СтрНайти(ТелефонЧатИд, "@g.us") Тогда
			Возврат ТелефонЧатИд
		Иначе
			Возврат Формат(Число(ТелефонЧатИд), "ЧГ=") + "@c.us"
		КонецЕсли;
	КонецЕсли;
