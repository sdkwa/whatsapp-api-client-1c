...
&НаКлиенте
Процедура ОбновитьQRКод(Команда)
...
&НаКлиенте
Процедура ПолучитьSMS(Команда)
	
	Если Не ЗначениеЗаполнено(НомерТелефона) Тогда
		Сообщить("Введите номер телефона", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
	
	ПолучитьКодСервер("sms");
	
КонецПроцедуры

&НаКлиенте
Процедура ЗапроситьЗвонок(Команда)
	Если Не ЗначениеЗаполнено(НомерТелефона) Тогда
		Сообщить("Введите номер телефона", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
	
	ПолучитьКодСервер("voice");
	
КонецПроцедуры

&НаКлиенте
Процедура ОтправитьКод(Команда)
	Если Не ЗначениеЗаполнено(КодовоеЧисло) Тогда
		Сообщить("Введите регистрационный код", СтатусСообщения.Внимание);
		Возврат;
	КонецЕсли;
...
Функция Подключаемый_СтраницаЧетыре_ПриПереходеДалее(Отказ)
	...
	ПроверитьНеобходимостьQRСервер();
...
&НаКлиенте
Процедура ЗарегистрироватьсяВSDKWA(Команда)
	Оповещение = Новый ОписаниеОповещения("ЗавершитьЗапускПриложенияGR", ЭтотОбъект);
	НачатьЗапускПриложения(Оповещение, "https://cabinet.sdkwa.pro");
КонецПроцедуры
...
&НаСервере
Процедура ПолучитьQRСеррвер()

	Ответ = ОбработкаОбъект().ПолучитьQRКод();
	Консоль = СтруктураВJSONСтроку(Ответ);
	СтатусQRКода = Ответ.type;
	Если Ответ.type = "qrCode" Тогда
		QRАвторизация = ПолучитьТекстИзМакета("QRМакет");
		QRАвторизация = СтрЗаменить(QRАвторизация, "%QR_DATA%", Ответ.message);
	ИначеЕсли Ответ.type = "alreadyLogged" Тогда
		Сообщить("Аккаунт уже авторизован");
		ПроверитьНеобходимостьQRСервер();
	ИначеЕсли Ответ.type = "error" Тогда
		Сообщить("Ошибка получения QR кода: " + Ответ.message);
	КонецЕсли;
...
&НаСервере
Процедура ПроверитьНеобходимостьQRСервер()

	СтатусСервисаТекст = ОбработкаОбъект().ПолучитьСтатусСервиса(ТекстОшибкиПодключения);
	СтатусСервисаАвторизован = (СтатусСервисаТекст = "authorized");
	
	Если СтатусСервисаАвторизован Тогда
		Элементы.ГруппаСписокАвторизация.Видимость = Ложь;
		Элементы.ГруппаАвторизацияНеТребуется.Видимость = Истина;
	Иначе
		Элементы.ГруппаСписокАвторизация.Видимость = Истина;
		Элементы.ГруппаАвторизацияНеТребуется.Видимость = Ложь;
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура ПолучитьКодСервер(ТипКода)

	Попытка
		Ответ = ОбработкаОбъект().ПолучитьКодРегистрации(НомерТелефона, ТипКода);
		
		Если Ответ.status = "sent" Тогда
			Элементы.СМСКодОтправлен.Видимость = Истина;
			Элементы.ОшибкаОтправкиСМСКода.Видимость = Ложь;
		ИначеЕсли Ответ.status = "fail" Тогда
			Элементы.СМСКодОтправлен.Видимость = Ложь;
			Элементы.ОшибкаОтправкиСМСКода.Видимость = Истина;
			Элементы.ОшибкаОтправкиСМСКода.Заголовок = СтрШаблон("Ошибка отправки СМС: %1. Повторить после: %2 сек", Ответ.reason, 
				? (Ответ.Свойство("retry_after"), Ответ.retry_after, Ответ.sms_wait));
			Сообщить(СтруктураВJSONСтроку(Ответ));
		Иначе
			Элементы.СМСКодОтправлен.Видимость = Ложь;
			Элементы.ОшибкаОтправкиСМСКода.Видимость = Истина;
			Элементы.ОшибкаОтправкиСМСКода.Заголовок = СтрШаблон("Ошибка отправки СМС: %1", Ответ.reason);
			Сообщить(СтруктураВJSONСтроку(Ответ));
		КонецЕсли;
	Исключение
		Рез = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Сообщить("Ошибка. Проверьте, чтобы аккаунт был создан на правильной версии сервера - BASIS V2 - Новые возможности");
	КонецПопытки;

КонецПроцедуры

&НаСервере
Процедура ОтправитьКодСервер()
	
	Элементы.ТелефонАвторизован.Видимость = Ложь;
	Элементы.ОшибкаАвторизации.Видимость = Ложь;
	
	Ответ = ОбработкаОбъект().ОтправитьКодРегистрации(КодовоеЧисло);
	Если ТипЗнч(Ответ) = Тип("Строка") Тогда
		Элементы.ОшибкаАвторизации.Видимость = Истина;
		Сообщить(Ответ);
	ИначеЕсли Ответ.status = "ok" Тогда
		Элементы.ТелефонАвторизован.Видимость = Истина;
	Иначе
		Сообщить(СтруктураВJSONСтроку(Ответ));
		Элементы.ОшибкаАвторизации.Видимость = Истина;
	КонецЕсли;
