...
Здесь содержится конфигурация, выгруженная через конфигуратор 1С в формате xml файлов. Используя конфигурацию можно отправлять и получать сообщения к себе на WhatsApp, проверять наличие Whatsapp, получать контакты, чаты, историю переписок с собеседниками.  Интеграция с WhatsApp сделана через REST сервис 
...
2. Через кнопку Помощник пройти авторизацию, либо сделать через сайт. Получить ``API Token`` и ``ID Instance``
...
Обработка имеет программный интерфейс по [стандартам разработки 1С](https://its.1c.ru/db/v8std). Ее можно встроить в конфигурацию и вызывать АПИ на сервере через создание объекта. Документация по всем методам находится по [ссылке]. Примеры использования:
